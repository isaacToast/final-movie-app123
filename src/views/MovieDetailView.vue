...
// ----------------------------------------------------
// [이번 주 추가 코드] AI 관련 상태 변수들
const aiReview = ref('') // AI가 쓴 글을 담을 변수
const isAiLoading = ref(false) // AI가 글을 쓰는 동안 띄울 로딩 상태

// AI에게 리뷰를 부탁하는 비동기 함수 (순수 Axios 포스트 요청 방식)
const generateAIReview = async () => {
  if (!store.selectedMovie) return

  isAiLoading.value = true
  aiReview.value = '' // 기존 글자 초기화

  try {
    const promptMessage = 
      `너는 영화 리뷰 유튜버야. 아래 영화 데이터를 기반으로 흡입력 있는 추천평을 3문단으로 작성해줘(이모지 필수). 
    ...
    const response = await axios.post(
      ...
      },
      {
        headers: {
          ...
          // .env에 안전하게 숨겨둔 환경변수 키를 헤더에 실어서 인증합니다.
          'Authorization': `Bearer ${import.meta.env.VITE_AI_API_KEY}`
        }
      }
    );

    // 표준 데이터 규격에 맞춰 생성된 텍스트 결과물만 쏙 낚아채서 변수에 담습니다.
    aiReview.value = response.data.choices[0].message.content;

  } catch (error) {
    console.error("AI 호출 에러:", error)
    aiReview.value = "AI 서비스와 연결하는 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요. 😢"
  } finally {
...
<template>
        ...
        <div class="info-zone">
          ...
          <div class="ai-section">
            <button @click="generateAIReview" class="ai-btn" :disabled="isAiLoading">
              {{ isAiLoading ? '🤖 AI가 열심히 글을 쓰는 중...' : '✨ AI의 30초 맞춤 추천사 듣기' }}
            </button>
  ...
  <div v-else-if="store.isLoading" class="full-screen-loading-gate">
    ...
    <p class="loading-text">시네마틱 데이터 센터로부터 정보를 퍼 올리는 중입니다...</p>
  </div>
...
<style scoped>
.detail-page { position: relative; min-height: 100vh; color: #ffffff; background-color: #0c1014;  }
.backdrop-layer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-size: cover; background-position: center top; z-index: 0; transform: scale(1.05); filter: blur(4px); }
.black-curtain { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(to right, #0c1014 25%, rgba(12, 16, 20, 0.8) 60%, #0c1014 100%), linear-gradient(to bottom, transparent 50%, #0c1014 100%); }
.content-container { position: relative; z-index: 1; max-width: 1200px; margin: 0 auto; padding: 50px 30px; }
.back-floating-btn { background: rgba(255, 255, 255, 0.15); border: 1px solid rgba(255, 255, 255, 0.2); color: #ffffff; padding: 12px 24px; border-radius: 40px; cursor: pointer; font-weight: 700; font-size: 15px; margin-bottom: 40px; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }
...
@media (max-width: 900px) { .movie-hero-grid { grid-template-columns: 1fr; gap: 40px; justify-items: center; } .poster-zone { width: 280px; } .movie-main-title { font-size: 38px; text-align: center; } .tagline { text-align: center; border-left: none; } }

/* AI 영역 디자인 */
.divider { border: 0; height: 1px; background: rgba(255, 255, 255, 0.2); margin: 30px 0; }
.ai-section { margin: 30px 0; padding: 20px; background: rgba(255, 255, 255, 0.1); border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); }
.ai-btn { background: linear-gradient(45deg, #8a2be2, #4b0082); color: white; border: none; padding: 12px 20px; font-size: 16px; font-weight: bold; border-radius: 8px; cursor: pointer; width: 100%; transition: 0.3s; }
.ai-btn:hover:not(:disabled) { box-shadow: 0 0 15px #8a2be2; transform: scale(1.02); }
.ai-btn:disabled { background: #555; cursor: not-allowed; }
.ai-result-box { margin-top: 15px; padding: 15px; background: #222; border-left: 4px solid #8a2be2; color: #fff; font-size: 16px; line-height: 1.6; border-radius: 0 8px 8px 0; }
</style>